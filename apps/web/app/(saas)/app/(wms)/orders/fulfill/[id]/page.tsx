...
export default function WaveDetailPage() {
	...
	const totalPicked = wave.lines.reduce(
						...
						<TableBody>
							{wave.lines.map((l) => (
								<TableRow key={l.id}>
									...
									<TableCell>
										{l.salesOrderLine?.lineNumber ?? "—"}
									</TableCell>
									<TableCell className="font-mono text-xs">
										{l.salesOrderLine?.sku?.sku ?? "—"}
									</TableCell>
									<TableCell>
										{l.salesOrderLine?.sku?.name ?? "—"}
									</TableCell>
									<TableCell className="text-right">
										{Number(l.qtyToPick)}
									</TableCell>
									<TableCell className="text-right">
										{Number(l.qtyPicked ?? 0)}
									</TableCell>
								</TableRow>
							))}
						</TableBody>
